...
class TPURL {
                ...
                Object.assign( this, source, {
                    ...
                    [ MANIPULATION ]:
                        ( thisManip && sourceManip ) ?
                            ...
                            thisManip || sourceManip,
                    [ PATH ]: this[ PATH ].concat( source[ PATH ] || [] ),
                } );
            }
        } else {
    ...
    dataSrc() {
        let src = this[ MULTIPLE ] ? this[ MULTIPLE ].encrypted : this[ SRC ];
        if ( !src ) {
    ...
    dataTransform( withFocus = false ) {
        ...
        const border = this[ BORDER ] ? `border=${ this[ BORDER ] }` : ``;
        const colors = join( background, border );
        ...
        const manip =
            this[ MANIPULATION ] ?
                ...
                ``;
        return join( manip, join( join( colors, filters ), join( format, truecolor ) ) );
    }
    host( host ) {
    ...
    path( ...args ) {
        return new this.constructor( this, {
            [ PATH ]: args.map( segment => `${ segment }`.trim() ),
        } );
    }
    toString() {
...
class Multiple extends TPURL {
    constructor( ...sources ) {
        super( ...sources );
        if ( this[ MULTIPLE ] && ( this[ MULTIPLE ].src !== this[ SRC ] ) ) {
            const { crypto, path } = this[ MULTIPLE ];
            this[ MULTIPLE ] = {
                crypto,
                "encrypted": this[ SRC ] && crypto.encrypt( this[ SRC ] ),
                path,
                "src": this[ SRC ],
            };
        }
    ...
    multiple() {
        throw new Error( `cannot call method multiple twice` );
    }
    // eslint-disable-next-line class-methods-use-this
    placeholder() {
        throw new Error( `cannot call method placeholder after method multiple was called` );
    }
}

TPURL.prototype.multiple = function( ...args ) {
    // eslint-disable-next-line no-magic-numbers
    if ( !args.length ) {
        throw new Error( `method multiple requires an encryption key` );
    }
    return new Multiple( this, {
        [ MULTIPLE ]: {
            "crypto": new Crypto( args[ 0 ] ),
            "encrypted": undefined,
            "src": undefined,
        },
    } );
...
class Placeholder extends TPURL {
    // eslint-disable-next-line class-methods-use-this
    multiple() {
        throw new Error( `cannot call method multiple after method placeholder was called` );
    }
    // eslint-disable-next-line class-methods-use-this
    placeholder() {
