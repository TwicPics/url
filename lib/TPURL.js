...
class TPURL {
    ...
    dataTransform( withFocus = false ) {
        ...
        const manip =
            this[ MANIPULATION ] ?
                ...
                ``;
        return join( manip, join( background, format ) );
    }
    host( host ) {
    ...
    toString() {
        ...
        const [ , protocol, path, queryValue = `` ] = rQuery.exec( src );
        const query = queryValue && `?${ queryValue }`;
        if (
            ( protocol !== `image:` ) ||
            rVersionPath.test( path ) ||
            ( !manip && rVersionPath.test( queryValue ) )
        ) {
            return `${ host }/${ VERSION }/${ manip && `${ manip }/` }${ protocol }${ path }${ query }`;
        }
...
// format
setterFactory( `format`, FORMAT, rFormat );
for ( const format of [ `jpeg`, `png`, `webp` ] ) {
    TPURL.prototype[ format ] = function() {
